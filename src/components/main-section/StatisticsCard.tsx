...
const StatisticsCard = () => {
  const { updatedRank, updatedPercentile, updatedScore } =
      ...
      <div>
        ...
        <ul className="sm:flex items-center gap-2 justify-around text-xs md:text-base">
          {contents.map((content, index) => (
            <li
              key={index}
              className={`flex items-center gap-2 py-2 md:pr-6 ${
                index < contents.length - 1 && "md:border-r"
              }`}
            >
              <div className="size-6 md:size-12 rounded-full border border-[#F0F2F5] flex items-center justify-center bg-[#F4F6F8] m-1 md:m-3">
                <span>{content.icon}</span>
              </div>
              <div>
                <h2 className="md:text-lg font-bold">{content.title}</h2>
                <p className="uppercase text-xs md:text-base text-[#9197A0]">
                  {content.text}
                </p>
              </div>
            </li>
